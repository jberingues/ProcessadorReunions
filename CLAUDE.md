...
uv sync --group experiments                      # deps extra per a scripts/ (faster-whisper)
```

Els experiments i utilitats de desenvolupament viuen a `scripts/` (proves de whisper, preview del PairingView, prova manual de l'extractor de projectes) — no formen part de l'app.
...
- **`calendar_matcher.py` — `CalendarMatcher`**: OAuth Calendar. `_parse_event` → `{title, start, end, duration, attendees}`; `start` tz-aware **només** per a events amb `dateTime`: els de dia sencer (només `date`) donarien datetimes naive, per això `CalendarWorker` els filtra amb `_is_timed_meeting` (naive vs aware petava el sort i el matching).
...
- **`obsidian_writer.py` — `ObsidianWriter`** + helper `series_name_for_file()`: totes les operacions al vault. `create_simple/meeting/email_note` (no pre-creen `Temes oberts.md`). `ensure_temes_oberts(series_dir)` el crea buit (`### Altres temes`) si falta, idempotent — usat per la fase 1. `find_existing_note` (considera els 4 sufixos `''`/`~`/`+`/`*`, evita re-imports). `find_pending_consolidation_notes` (notes `+`, font del Wizard Consolidar); `mark_as_ordre_generated` (`~`→`+`); `find_uncorrected_notes` exclou `~`/`+`/`*`. `read_attendees` / `read_attendees_string` (assistents del frontmatter; **únic punt de parsing** — el wizard de processat hi delega). `create_email_thread_note` (escriu fil sencer a `Correus/`, frontmatter `type:correu`, una secció per missatge, respostes amb l'històric citat retallat via `trim_quoted_reply`, adjunts idempotents). `append_to_year_note` = **destí unificat de tots els processats** (capçalera `## <date> - <title>` + `Assistents` + bloc; any del prefix YYMMDD); el fitxer anual duu frontmatter `type: resum_anual` + `serie` + `any` (es prepèn també als anuals antics que no en tinguin) per a cerques estructurades al vault. Migració one-shot dels existents: `scripts/migrate_year_frontmatter.py` (executada 2026-07-10; dry-run per defecte, `--apply`). `mark_as_corrected/processed` (`processed` accepta `~` o `+` com a estat previ → `*`). **`append_to_historic` / `append_email_to_provider_note` deprecated** (només `wizard_processar_correus.py`; no usar en codi nou).
- **`transcript_corrector.py` — `TranscriptCorrector(vocab, semantic_memory_path, model, threshold_auto=0.85)`**: `detect()` aplica memoritzades (globals del Vocabulari + locals del `semantic_memory.json`), crida LLM amb vocab+memòria+referència+few-shot de falsos positius, filtra confiança <0.85, després filtre fonètic (`is_likely_phonetic`, distància >0.75). **El pre-pass fuzzy es va ELIMINAR del corrector**: generava falsos positius (paraules catalanes comunes — `cosa`, `pots`, `Vila` — matchejaven amb termes a 0.7-0.85); el LLM ja veu el vocabulari sencer. Reemplaçament whole-word compartit (`_replace_whole_word`; insereix la correcció literalment — mai com a template de `re.sub`).
- **`phonetic_filter.py`**: `levenshtein/normalized_distance/similarity` (case/accent-insensitive). `is_likely_phonetic` **en ús**; `find_fuzzy_candidates` **no s'usa enlloc** (conservada només per experiments).
...
- **`gui/workers.py` — QThread Workers**:
  ...
  - Workers: `CalendarWorker` (filtra events de dia sencer via `_is_timed_meeting`; finestra en hora local amb offset, no `'Z'`), `EmailArchiveWorker` (sync+fetch+dispatch+escriptura; store desat incrementalment cada 10 fils; `summary` amb `archived_threads`, `skipped_*`, `sync_*`, `errors`), `GmailLabelSyncWorker` (només sync etiquetes), `Correction*`/`BatchCorrectionPrepareWorker`/`BatchCorrectionDetectWorker`, `DailyProcessorWorker`, `MeetingAnalyzerWorker` (`brief=True`), `SummaryWorker`, `ProjectInitWorker`, `PlaudListWorker`, `PlaudTranscriptWorker` (emet `file_id` per descartar resultats stale).
  - **`detach_worker(worker)` + `_DETACHED_WORKERS`**: un QThread dins de `run()` no es pot aturar (`quit()` no hi fa res) i destruir-lo viu amb el diàleg pare avorta l'app sencera. Quan un wizard abandona un worker en curs (Enrere / tancar), li desconnecta els senyals (el resultat tardà no ha d'escriure res) i el desvincula del pare cap a aquest registre, on queda fins que acaba. Usat per `wizard_processar` (`_release_running_worker` + `_confirm_close`), `wizard_transcripcio` (`_release_transcript_worker`) i `wizard_correccio` (helper `_release_worker`: abort + `wait(3000)` i, si expira, desconnexió + detach; s'aplica al worker de preparació i al de detecció, tant a Enrere com a tancar).
...
- **Sincro** (`DailyProcessorWorker`, CrewAI) — daily scrum per persona. Sense fase 2: escriu l'anual i marca `*`. La detecció de speakers a la transcripció reconeix dos formats de línia: Teams (`HH:MM:SS Nom`) i Plaud (`[MM:SS - MM:SS] Nom:`).
...
Pàg.1 batch: **dues fases, cap I/O al fil de la GUI**. `BatchCorrectionPrepareWorker` llegeix el vocabulari i, per nota, transcripció + referència + memòria semàntica (`build_if_stale`) i emet una tasca per nota (`Preparant i/n`); quan acaba, `_on_prepare_finished` arrenca el `BatchCorrectionDetectWorker` amb les tasques preparades. Notes sense correccions → corregides directament. Pàg.2 revisió: `InlineCorrectionEditor`; en desar actualitza `semantic_memory.json`, `Vocabulari.md` (alias + termes validats), transcript corregit, marca `~`.
...
**Preparació fora del fil principal** (decisió 2026-08-28): abans `_prepare_and_start_batch` feia tota la lectura del vault al slot del botó. El vault viu a Google Drive (CloudStorage): cada lectura pot ser una descàrrega i, amb la xarxa/VPN trontollant, quedar-se minuts bloquejada → finestra congelada sense cap pista (diagnosticat amb `sample <pid>`: fil principal dins d'un `read()` cridat des del clic). Regla general: **cap lectura del vault al fil de la GUI**. En avortar (Enrere/tancar) els dos workers es **desvinculen** amb `_release_worker` (abort + `wait(3000)` + `detach_worker`), no només s'aborten: una lectura penjada pot tornar tard i escriure dins d'un `batch_results` ja reiniciat.
