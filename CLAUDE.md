...
**Flux de seguiments en dues fases** (decisió 2026-06-15): el processat d'una reunió de seguiment es parteix en **fase 1** (Wizard Processar → genera *només* `Ordre del dia propera reunió.md`, nota `~`→`+`) i **fase 2** (Wizard Consolidar → l'usuari ha validat l'Ordre del dia a Obsidian; en propaga els resums a `Temes oberts.md` + fitxer anual, nota `+`→`*`). El gate de validació humà entre fases garanteix que la memòria permanent és fidel (els errors de transcripció es cacen abans de consolidar). Sincro NO té fase 2 (escriu l'anual directament `~`→`*`).
...
| Processar reunions | `wizard_processar.py` | **Fase 1**. Selector per fila (3 opcions: Resum+ordre dia / breu / Sincro); default `Sincronització/`→Sincro, resta→Resum+ordre dia. Seguiments escriuen *només* `Ordre del dia propera reunió.md` i marquen `+`. Sincro escriu l'anual i marca `*`. |
...
2 pàgines. Pàg.0: taula de notes corregides, `QComboBox` per fila amb 3 opcions (l'opció "Resum" simple es va eliminar 2026-06-15 amb la unificació):
- **Resum+ordre dia** (`MeetingAnalyzerWorker`, CrewAI) — compara amb `Temes oberts.md` i escriu **només** `Ordre del dia propera reunió.md`. **Ja NO toca Temes oberts ni l'anual** (això és fase 2). Marca la nota `+`.
...
1 pàgina (`wizard_consolidar.py`). Llista les notes `+` (`find_pending_consolidation_notes`); l'usuari selecciona i clica "Consolidar seleccionades". Per cada nota: `consolidate_pending_note` (a `consolidator.py`) llegeix l'`Ordre del dia` **ja validat per l'usuari a Obsidian**, el parseja (`parse_ordre_del_dia`) i propaga els resums a `Temes oberts.md` (bullets datats) + `<Any> <Subfolder>.md` (bloc anual), després marca `*`. **Síncron** (sense LLM — és parse + escriptures).
