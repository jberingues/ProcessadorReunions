...
| Processar reunions | `wizard_processar.py` | **Fase 1**. Selector per fila (3 opcions: Resum+ordre dia / breu / Sincro); default `Sincronització/`→Sincro, resta→Resum+ordre dia. Seguiments escriuen *només* `Ordre del dia - <sèrie>.md` i marquen `+`. Sincro escriu l'anual i marca `*`. |
...
1 pàgina (`wizard_consolidar.py`). Llista les notes `+` (`find_pending_consolidation_notes`); l'usuari selecciona i clica "Consolidar seleccionades". Per cada nota: `consolidate_pending_note` (a `consolidator.py`) llegeix l'`Ordre del dia` **ja validat per l'usuari a Obsidian**, el parseja (`parse_ordre_del_dia`) i propaga els resums a `Temes oberts.md` (bullets datats) + `<Any> <Subfolder>.md` (bloc anual), treu el frontmatter `pendent_revisio` de l'Ordre del dia, i marca `*`. **Síncron** (sense LLM — és parse + escriptures).
