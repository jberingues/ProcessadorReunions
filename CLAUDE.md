...
*Retry de xarxa*: el mòdul exposa `_is_transient_network_error(exc)` i `_retry_on_network_error(fn, attempts=3, base_delay=0.6)`. El segon executa `fn()` reintentant amb backoff lineal davant errors de socket transitoris (`EADDRNOTAVAIL` = Errno 49 a macOS — "Can't assign requested address", típic quan l'app arrenca mentre la xarxa/VPN encara s'estabilitza — més `ECONNRESET`/`ETIMEDOUT`/`socket.timeout`/`ConnectionError`/etc.). Re-llança immediatament si l'error **no** és transitori (auth, quota…) o si s'esgoten els intents. S'aplica a les crides de xarxa **single-shot** que tomben tota l'operació si fallen: `CalendarWorker` (`events().list`), `GmailLabelSyncWorker` (`sync_gmail_labels`) i les crides de setup d'`EmailArchiveWorker` (`sync_gmail_labels`, `list_user_labels`, `list_thread_ids_for_day`). El bucle per fil d'`EmailArchiveWorker` (`peek_thread`/`fetch_thread_full`) **no** porta retry: ja degrada bé per fil (salta i continua) i re-executar el wizard és idempotent.
